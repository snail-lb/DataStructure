...
public class BinaryTree<E> {
	 ...
	 * @param node
	 */
	private void  preorderTraversalRealize(Node<E> node){
		if(node != null){
	 ...
	 * @param node
	 */
	private void  inorderTraversalRealize(Node<E> node){
		if(node != null){
	 ...
	 * @param node
	 */
	private void  postorderTraversalRealize(Node<E> node){
		if(node != null){
	...
	/**
	 * 广度优先搜索(分层遍历二叉树):
	 * 使用队列实现。队列初始化，将根节点压入队列。当队列不为空，
	 * 进行如下操作：弹出一个节点，访问，若左子节点或右子节点不为空，将其压入队列。
	 * @param node
	 * @return
	 */
	public Object[] layerTraversing(Node<E> node){
	 ...
	 * @return
	 */
	public Object[] depthTraversing(Node<E> node){
		List<E> list = new ArrayList<E>();
